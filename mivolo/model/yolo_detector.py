...
class Detector:
    ...
    def predict_batch(self, images: Union[List[np.ndarray], List[str], List["PIL.Image"]]) -> List[PersonAndFaceResult]:
        batch_results: List[Results] = self.yolo.predict(images, **self.detector_kwargs)
        return [PersonAndFaceResult(res) for res in batch_results]
