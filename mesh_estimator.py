...
class HumanMeshEstimator:
    def __init__(self, device='cuda', smpl_model_path=SMPL_MODEL_PATH, threshold=0.25):
        self.device = device
    ...
    def init_cam_model(self):
        model = FLNet()
        checkpoint = torch.load(CAM_MODEL_CKPT, weights_only=False)['state_dict']
    ...
    def get_output_mesh_with_transl(self, params, pred_cam, batch):
        img_h, img_w = batch['img_size'][0]
        cam_trans = self.convert_to_full_img_cam(
            pare_cam=pred_cam,
            bbox_height=batch['box_size'],
