...
from utils.matching import match_2d_greedy
...
def joint_mapping(source_format, target_format):
    mapping = np.ones(len(target_format),dtype=np.int64)*-1
...
class RH_Evaluation(object):
    def __init__(self, annots, results_path, RH_dir, device, smpl_model, set_name='test', subset=False):
        super(RH_Evaluation, self).__init__()
        self.set_name = set_name
        self.load_gt(RH_dir)
        self.human_model = smpl_model
        self.collect_results(annots, device)
        self.results_txt_save_path = results_path.replace('.npz', f'_results{"_subset" if subset else ""}.txt').replace('data/', 'result/')

        self.kp2d_mapper_BK = joint_mapping(BK_19, Crowdpose_14)
        self.kp2d_mapper_OCH = joint_mapping(OCHuman_19, Crowdpose_14)
        self.match_kp2ds(subset)
    ...
    def miss_mat(self, miss_gt_ids):
        return np.stack([np.ones(len(miss_gt_ids))*-1, miss_gt_ids],1).astype(np.int64)

    def match_kp2ds(self, subset):
        self.match_results = {}
        self.missed_ids = {}
        ...
        for img_name in self.annots.keys():
            ...
            if isinstance(results, list):
                pred_kp2ds = np.array([r['kp2ds'][:, :, :2] for r in results])
            elif isinstance(results, dict):
                pred_kp2ds = results['kp2ds'][:, :, :2]
    ...
    def collect_results(self, annots, device):
        print('loading results ...')
        results = {}
        J_regressor_extra = np.load('data/models/smpl/J_regressor_extra.npy')
        J_regressor_extra = torch.tensor(J_regressor_extra, dtype=torch.float32, device=device)
        for imgname, pred_data in annots.items():
            if 'pose_cam' not in pred_data or len(pred_data['pose_cam']) == 0:
                print(f'No pose in {imgname}, skip it.')
                continue

            pred_pose = torch.tensor(pred_data['pose_cam'], dtype=torch.float32, device=device)
            pred_beta = torch.tensor(pred_data['shape'], dtype=torch.float32, device=device)
            pred_transl = torch.tensor(pred_data['trans_cam'], dtype=torch.float32, device=device)
            pred_intrinsics = torch.tensor(pred_data['cam_int'][0], dtype=torch.float32, device=device)
            # filter nan
            valid_mask = ~torch.isnan(pred_transl).any(-1)
            pred_pose = pred_pose[valid_mask]
            pred_beta = pred_beta[valid_mask]
            pred_transl = pred_transl[valid_mask]

            if pred_beta.shape[-1] == 10: # supplement 10 shape coeffs to 11
                pred_beta = torch.cat([pred_beta, torch.zeros_like(pred_beta[:, :1])], dim=-1)

            with torch.no_grad():
                smpl_output = self.human_model(
                    betas = pred_beta,
            ...
            pred_verts = smpl_output.vertices
            pred_j3ds = smpl_output.joints

            pred_j3ds_extra = torch.einsum('bik,ji->bjk', [pred_verts - pred_transl[:,None,:], J_regressor_extra]) + pred_transl[:,None,:]
            pred_j3ds = torch.concat([pred_j3ds[:, :25], pred_j3ds_extra], dim=1)
            ...
            pred_j2ds = pred_j2ds[:, [5,2,6,3,7,4,12,9,13,10,14,11,28,27]]
            results[imgname] = {
                "kp2ds": pred_j2ds.cpu().numpy(),           # (N_person, 14, 2)
                "trans": pred_transl.cpu().numpy(),           # (N_person, 3)
            }
        self.results = results

def evaluate_rh(results_path):
    print('Evaluating results in {}'.format(results_path))
    Relative_Human_dir = 'data/RelativeHuman'

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    annots = np.load(results_path, allow_pickle=True)
    ...
    annots = {'/'.join(k.split('/')[-1:]): v for k, v in annots.items()}
    ...
    RH_Evaluation(annots, results_path, Relative_Human_dir, device, smpl_model, set_name='test')
    

if __name__ == "__main__":
    results_path = 'data/relativehuman_test.npz'
    evaluate_rh(results_path)